...
export default class Artist extends Component {
    ...
    //Calls getArtistID first so getMainArtistInfo can be called
    componentDidMount() {
        this.getArtistId();
        this.getMainArtistInfo();
    }

    //This is used if the route is updated
    //It checks to see if the updated id is not the same as the artistId
    //If not it changes the artistId to be the updated id
    //And then calls getMainArtistInfo and updates the entire component
    //It is used in conjunction with getDerivedStateFromProps to update the component
    componentDidUpdate(nextProps) {
        if (nextProps.match.params.id !== this.state.artistId) {
            this.setState({
                artistId: nextProps.match.params.id
            })
            this.getMainArtistInfo();
        } else {
            return null;
    ...
    //It checks to see if the updated id is not the same as the artistId
    //if not it changes the artistId to be the updated id
    //It is used in conjunction with componentDidUpdate to update the component
    static getDerivedStateFromProps(nextProps, prevState) {
        if (nextProps.match.params.id !== prevState.artistId) {
            return { artistId: nextProps.match.params.id }
        }
    }

    //This is used to get the artistId from the URL parameters
    //The Id is put in after the artist part of the URL
    //For example, here is ozzy Osbourne's URL : "http://localhost:3000/artist/'8aa5b65a-5b3c-4029-92bf-47a544356934'"
    //The part in single quotes is the artistId
    getArtistId = () => {
        let url = window.location.href;
        let artistUrl = url.split('/');
        this.state.artistId = artistUrl[4];
    }
    ...
    getMainArtistInfo = () => {
        axios.get('http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=' + LAST_FM_KEY + '&format=json&mbid=' + this.state.artistId)
            .then(res => {
...
getSimilarArtistInfo = () => {
    ...
    for (i = 0; i < this.state.similarArtists.length; i++) {
...
//I learned about dangerouslySetInnerHTML here: https://zhenyong.github.io/react/tips/dangerously-set-inner-html.html
                ...
                <div class="col-sm-4">
                    <h3 className="center-data">Similar artists:</h3>
                    {
                        this.state.similarArtistInfo.length > 0 ?
                            <ul className="lists">
                                {this.state.similarArtistInfo.map((artist, key) => {
